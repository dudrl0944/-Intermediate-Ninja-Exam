...
// this is the in-memory version of the BTreeNode
{
  public:
	...
	//추가구현 start
	int CopyNode(BTreeNode<keyType> *fromNode); //  노드를 복사하는 함수
	keyType Findnextkey(keyType key);  //  노드 내에서 입력된 키 다음 키를 찾는 함수
	keyType Findprevkey(keyType key); // 노드 내에서 입력된 키 이전 키를 찾는 함수
	int RMerge(BTreeNode<keyType> *fromNode);  // 추가 구현
	int FSplit(BTreeNode<keyType> * newNode); // 추가 구현
	void TPrint(ostream & stream) const;
	//추가구현 end
